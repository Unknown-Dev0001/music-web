...
const Languages = () => {
  ...
  return (
    <div className=" text-white pt-10 m-2 rounded-md w-[99%] hover:bg-white/5">
      <details className="text-white detailanimatation">
        ...
        <form className=" grid grid-cols-2 mb-1 h-25 overflow-y-scroll overflow-x-hidden">
          {languageList.map((language) => (
            <div key={language.id} className="flex items-center mb-3 mx-2 ml-3">
              <input
                type="checkbox"
                id={language.id}
                name="language"
              ...
              <label
                ...
                } cursor-pointer transition-colors text-sm min-w-[99px] text-center  border rounded-md p-2 font-semibold`}
              >
                {language.label}
              </label>
            </div>
